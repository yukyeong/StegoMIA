...
def load(name, pretrained = False):
    ...
    try:
        model = build(jit_model.state_dict(), pretrained=pretrained)
    except (KeyError, AttributeError) as e:
        # Handle case where state_dict might be nested or have different structure
        state_dict = jit_model.state_dict()
        if "state_dict" in state_dict:
            state_dict = state_dict["state_dict"]
        # Remove module prefix if present
        if any(key.startswith("module.") for key in state_dict.keys()):
            state_dict = {key[len("module."):] if key.startswith("module.") else key: value 
