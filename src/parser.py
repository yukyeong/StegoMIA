...
def parse_args():
    ...
    parser.add_argument("--model_name", type = str, default = "RN50", choices = ["RN50", "RN101", "RN50x4", "ViT-B/32", "ViT-B/16", "ViT-L/14"], help = "Model Name")
    parser.add_argument("--train_data", type = str, default = None, help = "Path to train data csv/tsv file")
    parser.add_argument("--train_token_file", type=str, default=None, help="Path to training token file (image#idx\\tcaption)")
    ...
    parser.add_argument("--finetune_evaluation", action = "store_true", default = False, help = "Finetune classification")
    ...
    parser.add_argument("--eda_prob", type=float, default=0.1, help='patch scale relative to image')
    parser.add_argument("--aug_prob", type=float, default=0.1, help='patch scale relative to image')
    ...
    parser.add_argument("--log_dir_path", type = str, default = None, help = "Target label of the backdoor attack")
    
    # Fine-tuning specific arguments
    parser.add_argument("--test_image_dir", type=str, default=None, help="Directory containing test images for validation")
    parser.add_argument("--annotations_file", type=str, default=None, help="Path to annotations file for validation")
    parser.add_argument("--reference_image_dir", type=str, default=None, help="Directory containing reference images for quality metrics alignment")
    parser.add_argument("--output_results_file", type=str, default=None, help="Path to output results file")
    parser.add_argument("--final_model_name", type=str, default=None, help="Name for final saved model")
    parser.add_argument("--checkpoints_dir_path", type=str, default=None, help="Directory to save checkpoints")
    
    # Dataset type flags
    parser.add_argument("--train_is_mscoco", action="store_true", default=False, help="Use MSCOCO dataset format for training")
    parser.add_argument("--train_is_imagenet1k", action="store_true", default=False, help="Use ImageNet-1k dataset format for training")
