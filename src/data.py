...
def apply_trigger(image, **kwargs):
    return image

# Import augmentation defense module
try:
    from src.defense.augmentation import (
        AugmentationDefense,
    ...
    logging.warning("Augmentation defense module is not available")
...
class ImageCaptionDataset(Dataset):
    def __init__(self, path, image_key, caption_key, delimiter, processor, inmodal = False, defense = False, crop_size = 150):
        ...
        self.defense_transform = None
        if self.defense:
            if DEFENSE_AVAILABLE:
                self.defense_transform = AugmentationDefense(
                    crop_size=crop_size,
                    input_size=224,
                ...
                ).get_train_pil_transform()
            else:
                logging.warning("Defense requested but augmentation defense module unavailable")
    ...
    def __getitem__(self, idx):
        ...
        if self.defense_transform is not None:
            image = self.defense_transform(image)
        ...
        if(self.inmodal):
            ...
            augmented_img = _augment_image(os.path.join(self.root, self.images[idx]))
            if self.defense_transform is not None:
                augmented_img = self.defense_transform(augmented_img)
            item["pixel_values"] = processed_image, self.processor.process_image(augmented_img)
        else:  
            item["input_ids"] = self.captions["input_ids"][idx]
            item["attention_mask"] = self.captions["attention_mask"][idx]
            item["pixel_values"] = processed_image
